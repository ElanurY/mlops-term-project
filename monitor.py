...
# ── Simulated Monitoring Batches (Data Drift Simulation) ─────────────────────
np.random.seed(99)

batches = {
    "week_1_normal":       {"noise": 0.0},
    "week_2_slight_drift": {"noise": 0.3},
    "week_3_high_drift":   {"noise": 0.8},
}
...
for batch_name, cfg in batches.items():
    ...
    y_new  = np.random.binomial(1, 0.5, n)
    ...
    print(f"{batch_name:28s} -> F1: {f1:.3f}  AUC: {auc:.3f}  ACC: {acc:.3f}")
